...
public class EventoPublisher {
  ...
  public void publicar(MensajeEvento evento) {
    try {
      rabbitTemplate.convertAndSend(exchange, routingKey, evento);
      log.info("Evento publicado en '{}' con routing key '{}': {}", exchange, routingKey, evento.titulo());
    } catch (AmqpException e) {
      // best-effort: si falla la publicación, no debería cortar el flujo del servicio
      log.error("No se pudo publicar el evento '{}': {}", evento.titulo(), e.getMessage());
    }
  }
}
